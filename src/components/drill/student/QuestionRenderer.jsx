...
const QuestionRenderer = ({
  ...
  return (
    <div className="min-h-screen fixed inset-0 z-50 overflow-y-auto bg-cover bg-fixed" style={{ backgroundImage: `url(${drillBg})` }}>
      ...
      <div className={`w-full flex flex-col ${isSmallScreen ? 'items-center justify-center px-4 py-4' : 'items-center justify-start mt-2'}`}>
          ...
          <div className="w-full flex flex-col items-center justify-center">
            ...
            <div className="w-full max-w-sm">
              <div className="relative bg-white rounded-2xl p-4 shadow-lg">
                <div className="text-sm font-semibold text-center">{currentQuestion.text}</div>
        ...
        {showTimer && currentQuestion && (
          <div className="flex justify-center mt-4">
            {(() => {
              ...
              const penaltyThreshold = isMemoryGame ? 20 : 15; // Memory games have 20s threshold, others 15s
      ...
      {(isTeacherPreview || answerStatus === 'correct') && (
        <button
          className={`fixed bg-[#f39c12] text-white rounded-xl font-bold hover:bg-[#e67e22] shadow-lg transition-all hover:scale-105 ${isSmallScreen ? 'bottom-6 right-6 px-5 py-3 text-sm' : 'bottom-12 right-12 px-10 py-4 text-xl'}`}
          onClick={onNext}
        >
          Next
        </button>
